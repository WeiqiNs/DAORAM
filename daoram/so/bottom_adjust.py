...
class BottomUpSomapFixedCacheAdjust(BottomUpSomapFixedCache):
    """
    Bottom-to-Up SOMAP adjustment based on fixed-cache variant.

    Adjustments:
    1) Initialize with cache_size=c, but internally use c/2.
    2) Each access pushes one element into O_W and Q_W, while
       O_W and Q_W pop two elements. The popped elements move to
       O_R and Q_R, and O_R/Q_R pop three elements for deletion.
    3) O_b tracks elements in O_W and gen_available returns a key
       not in O_W.
    """
    ...
    def __init__(self,
                 ...
                 use_encryption: bool = True,
                 order: int = 4):
        self._raw_cache_size = cache_size
        adjusted_cache_size = max(1, cache_size // 2)
        super().__init__(
            num_data=num_data,
        ...
        # O_b: track elements currently in O_W
        self._Ob = set()
        self._ob_counter = 0

    def gen_available(self) -> Optional[int]:
        """Return a key not in O_W (O_b)."""
        if self._num_data <= 0:
            return None

        for i in range(self._num_data):
            candidate = (self._ob_counter + i) % self._num_data
            if candidate not in self._Ob:
                self._ob_counter = (candidate + 1) % self._num_data
                return candidate

        return None

    def _normalize_qw_item(self, item: Any) -> Tuple[Any, str]:
        if isinstance(item, tuple) and len(item) == 2:
            return item
        return (None, "Dummy")

    def _normalize_qr_item(self, item: Any) -> Tuple[Any, int, str]:
    ...
    def access(self, key: Any, op: str, value: Any = None) -> Any:
        ...
        # Retrieve key-value pair from both caches in parallel (h rounds)
    ...
    def _adjust_somap(self) -> None:
        # Pop two elements from Q_W/O_W
        qw_items = []
        for _ in range(2):
            if self._Qw_len > 0:
                item = self.operate_on_list(label=self._Qw_name, op="pop")
                self._Qw_len -= 1
            else:
                item = None
            qw_items.append(self._normalize_qw_item(item))

        for qw_key, qw_marker in qw_items:
            if qw_marker == "Key" and qw_key is not None:
                value = self._Ow.delete(qw_key)
                self._Ob.discard(qw_key)
                self._Ds.operate_on_key(op="w", key=qw_key, value=value)
                tmp = self._Or.search(qw_key)
                if tmp is None:
                    self._Or.insert(qw_key, (value, self._timestamp))
                else:
                    self._Or.search(qw_key, (value, self._timestamp))
                self.operate_on_list(label=self._Qr_name, op="insert",
                                     data=(qw_key, self._timestamp, "Key"))
            else:
                value = self._Ow.delete(None)
                self._Ds.operate_on_key(op="w", key=None, value=value)
                self._Or.insert(None)
                self.operate_on_list(label=self._Qr_name, op="insert",
                                     data=(None, self._timestamp, "Dummy"))
            self._Qr_len += 1

        # Pop three elements from Q_R/O_R for deletion
        for _ in range(3):
            if self._Qr_len > 0:
                qr_item = self.operate_on_list(label=self._Qr_name, op="pop")
                self._Qr_len -= 1
            else:
                qr_item = None

            qr_key, qr_ts, qr_marker = self._normalize_qr_item(qr_item)
            if qr_marker == "Dummy":
                self._Or.search(None)
                self._Or.delete(None)
            else:
                match_or = self._Or.search(qr_key)
                if match_or is not None and match_or[1] == qr_ts:
                    self._Or.delete(qr_key)
                else:
                    self._Or.delete(None)
