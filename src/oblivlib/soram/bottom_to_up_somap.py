...
from oblivlib.dependency.crypto import AesGcm
from oblivlib.dependency.interact_server import InteractServer, ServerStorage
from oblivlib.omap.avl_omap import AVLOmap
from oblivlib.omap.avl_omap_cache import AVLOmapCached
from oblivlib.oram.static_oram import StaticOram
...
class BottomUpSomap:
    ...
    def operate_on_list(self, label: str, op: str, pos: int = None, data: Any = None) -> Any:
        ...
        if op == 'insert':
            ...
            encrypted_data_list = self._client.add_read_list(label=label, indices=None)
            result = self._client.execute()
            encrypted_data_list = result.results[label]
