...
from oblivlib.dependency import AesGcm, InteractServer, Data
from oblivlib.dependency.config import AvlOmapCachedConfig, MulPathOramConfig
from oblivlib.dependency.graph import Graph
from oblivlib.omap.avl_omap_cache import AVLOmapCached
from oblivlib.oram.mul_path_oram import MulPathOram
...
class GraphOS:
    def __init__(self,
        ...
        # Build an encryptor when encryption is requested (None means plaintext storage).
        encryptor = AesGcm(key=aes_key, key_byte_length=num_key_bytes) if use_encryption else None

        # Initialize the OMAP.
        self._omap = AVLOmapCached(AvlOmapCachedConfig(
            client=client,
            num_data=num_data,
            key_size=key_size,
            data_size=data_size,
        ...
        self._oram = MulPathOram(MulPathOramConfig(
            client=client,
            num_data=num_data,
            data_size=key_size,
            bucket_size=bucket_size,
    ...
    def lookup(self, key: List[Any], update: Optional[int] = None) -> dict:
        """
